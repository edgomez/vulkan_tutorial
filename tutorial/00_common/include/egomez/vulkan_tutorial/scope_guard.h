...
static const scope_guard<aF>
makeScopeGuard(const aF& f)
{
...
#define SCOPE_GUARD_NAMED(varname, f)                                          \
    auto varname = ::egomez::vulkan_tutorial::makeScopeGuard(f)

#define SCOPE_GUARD(f)                                                         \
    SCOPE_GUARD_NAMED(SCOPE_GUARD_NAME_CONCAT(a_scope_guard_L, __LINE__), f)
